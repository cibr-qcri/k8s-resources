...
def write_sql_to_files():
    with open('/blockchain-parser/blocks_sql.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(block_buffer)
    with open('/blockchain-parser/tx_sql.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(transaction_buffer)
    with open('/blockchain-parser/in_addr_sql.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(in_address_buffer)
    with open('/blockchain-parser/out_addr_sql.csv', 'w', newline='') as file:
        writer = csv.writer(file)
...
def load_data(dict_name):
    wallet_file = Path("/blockchain-parser/" + dict_name + ".pickle")
    if wallet_file.exists():
        with open("/blockchain-parser/" + dict_name + '.pickle', 'rb') as f:
...
def load_processed_exchange_metadata():
    try:
        if Path("/blockchain-parser/processed_exchange_dates.pickle").exists():
...
def save_exchange_data():
    with open('/blockchain-parser/usd_exchange_rates.pickle', 'wb') as f:
    ...
    if COINDESK_END_DATE != last_process_date:
        ...
        with open('/blockchain-parser/processed_exchange_dates.pickle', 'wb') as f:
...
def main():
    ...
    with open("/blockchain-parser/blocks.json", "r") as f:
    ...
    with open("/blockchain-parser/enriched_transactions.json", "r") as f:
